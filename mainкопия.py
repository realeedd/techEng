...
class Testing(QMainWindow):
    def __init__(self, connection, userId):
        ...
        self.onMainAction()
        # подключаем меню
        self.mainMenu = QMenu("О приложении", self)
...
class MainPage(QWidget):
    def __init__(self, connection, userId, testWindow):
        ...
        dayWord = ''
        disc = ''
        dayWord = choice(self.words)
        disc = self.disc[self.words.index(dayWord)]
        self.dayWord.setText(dayWord)
        self.dayWordDisc.setText(disc)
    # показываем обновленные данные пользователей
    def refresh(self):
        cur = self.con.cursor()
        pointsTest = cur.execute('''SELECT IFNULL(sum(points), 0) FROM test_results
                       WHERE user_id = ?''', (self.id,)).fetchall()[0][0]
        ...
        self.pointTheoryLable.setText(f'Прогресс теории: {pointsTheory}')
    ...
    def chooseWord(self):
        ...
        try:
            if len(self.left) > 0:
                ...
                self.con.commit()
                QMessageBox.information(self, 'Тест пройден', f"Набрано баллов: {points} из 150")
    ...
    def chooseWord(self):
        ...
        try:
            if len(self.left) > 0:
                ...
                self.con.commit()
                QMessageBox.information(self, 'Тест пройден', f"Набрано баллов: {points} из 140")
        except Exception as error:
            print(error)
...
class ComputerTestWindow(QWidget):
    ...
    def check(self):
        try:
            ...
            self.con.commit()
            self.points.setText(f'Набрано баллов: {points}')
    ...
    def chooseWord(self):
        ...
        try:
            if len(self.left) > 0:
                ...
                self.con.commit()
                QMessageBox.information(self, 'Тест пройден', f"Набрано баллов: {points} из 90")
...
class TermsWindow(QWidget):
    def __init__(self, connection, userId, theoryWindow):
        ...
        self.theoryWindow = theoryWindow
        self.readButton.clicked.connect(self.readCheck)
        style = '''QPushButton{background-color:  rgb(89, 222, 118);
                border-radius: 20px;
                border-style: outset;
                 border-width: 4px;
                border-color:rgb(191, 191, 191);
