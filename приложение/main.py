...
from PyQt5.QtWidgets import QApplication, QMainWindow, QMessageBox, QWidget, QAction, QMenu


#окно где открывается главное окно, окно регистрации и входа
class OpenWindow(QMainWindow):
    ...
    def showMain(self):
        ...
        mainWindow.show()
    #кнопка показывающая окно регистрации
    def regButton(self):
    ...
    # #кнопка показывающая окно входа
    def signButton(self):
...
#пустое окно с меню, где открываются все окна
...
#окно теста со словами для разработки
...
class TermsTestWindow(QWidget):
    ...
    def reset(self):
        ...
        self.answerCount = 0
        # индексы слов для тестов
        self.left = [int(i) for i in range(0, len(self.sentence))]
